...
import SnippetCreateProjectPython from "/snippets/code/python/how-to-guides/metrics/local-metric/create-project.mdx";
...
import SnippetFullExperimentPython from "/snippets/code/python/how-to-guides/metrics/local-metric/full-experiment.mdx";
...
Every experiment in Galileo must live inside a **project**, which contains your datasets, experiments, and metrics.  

For this tutorial, set a project name in your `.env` file under the variable `GALILEO_PROJECT`.  
If the project does not already exist, the script will create it automatically.  
If the name is already in use, you’ll see a clear error instructing you to update `GALILEO_PROJECT` to a new value.
...
<Accordion title="The full experiment.py code">
<SnippetFullExperimentPython/>
</Accordion>
...
![Expanded trace showing the Terseness metric in the metrics pane](/how-to-guides/metrics/create-local-metric/expanded-trace-metrics-pane.webp)
