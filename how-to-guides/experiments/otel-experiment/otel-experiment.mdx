...
The function should handle both `str` and `dict` inputs, since the experiment runner passes the deserialized row data which can be in either format depending on your dataset structure.
...
| `dataset` | A list of dicts with `input` and optional `output` (ground truth) fields, a Galileo `Dataset` object, or a dataset name |
...
| `project` | Project name (can also be set via `GALILEO_PROJECT` env var) |
