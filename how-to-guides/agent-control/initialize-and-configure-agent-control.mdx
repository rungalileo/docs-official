...
<Note>
Set `AGENT_CONTROL_RUNTIME_TOKEN_HEADER` to your runtime token and use Agent Control SDK 8.5.0 or later. With this configuration, the SDK sends the runtime token in that header as a raw token without a `Bearer` prefix. `Authorization` remains reserved for gateway authentication.
</Note>
