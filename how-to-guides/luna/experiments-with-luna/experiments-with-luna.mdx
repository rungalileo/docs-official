...
This guide shows you how to use Luna 2 metrics in your experiments. This guide shows how to evaluate for prompt injection using an experiment with a dataset that contains 2 entries - one with a prompt injection, and one without. You will run the experiment using an LLM as a judge, then again using Luna 2.
...
- A [Galileo project](/concepts/projects) configured to use both an integration with an LLM, as well as the Luna 2 model
...
This code uses the prompt injection metric using LLM as a judge, leveraging whatever LLM integration you have set up. For example, if you have an OpenAI integration, it will use a model like GPT-4o.
...
Select each item to see the details of the experiment, including the results of the prompt injection metric.
...
The Luna 2 metrics are different metrics, rather than the same metric configured with a different LLM as the judge. To use the Luna metric, update the run experiment call:
...
- [Luna metrics](/concepts/metrics/metrics-reference#luna-metrics)
