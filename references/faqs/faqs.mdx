...
Install the Galileo SDK using the follow terminal command.
...
By initializing a new [Trace](/concepts/traces), the AI activity executed in the subsequent code is recorded to the selected Project's Log Stream.
...
### Q: How can I annotate or categorize different experiments my logs?
...
[Learn more about logging Traces →](/concepts/traces)

### Q: How do I retrieve data I’ve previously logged to Galileo via the API?

The API includes endpoints for retrieving logs, metrics, and datasets. For example, to fetch a Log Stream:

```bash
curl -X GET https://api.galileo.ai/v1/projects/{projectId}/logstreams/{logStreamId}/logs \
  -H "Authorization: Bearer YOUR_API_KEY"
```

[Learn more about Log Streams →](/concepts/logstreams)
...
[Metadata formatting tips →](/concepts/annotations)
...
2. Use `galileo_context` or create a new [Trace](/concepts/traces) in your application.
