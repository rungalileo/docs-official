...
// API Reference icon theme switching
(function () {
  function updateApiReferenceIcon() {
    // Find all API Reference icons in navigation
    const apiIcons = document.querySelectorAll('img[src*="api-reference.svg"]');
    
    if (apiIcons.length === 0) return;
    ...
    const iconPath = isDarkMode ? '/logo/api-reference-dark.svg' : '/logo/api-reference.svg';
    
    // Update all API Reference icons
    apiIcons.forEach(icon => {
      if (icon.src !== icon.src.split('/').slice(0, -1).join('/') + iconPath) {
        icon.src = iconPath;
      }
    });
  }
  
  function initApiIconSwitcher() {
    // Initial update
    updateApiReferenceIcon();
    ...
    // Also watch for new API icons being added
    const iconObserver = new MutationObserver((mutations) => {
      ...
      mutations.forEach((mutation) => {
        if (mutation.addedNodes && mutation.addedNodes.length) {
          for (let i = 0; i < mutation.addedNodes.length; i++) {
            const node = mutation.addedNodes[i];
            if (node.nodeType === 1 && 
                (node.tagName === 'IMG' && node.src && node.src.includes('api-reference.svg')) ||
                (node.querySelector && node.querySelector('img[src*="api-reference.svg"]'))) {
              shouldUpdate = true;
              break;
            }
          }
      ...
      if (shouldUpdate) {
        setTimeout(updateApiReferenceIcon, 100);
      }
    });
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", initApiIconSwitcher);
  } else {
    initApiIconSwitcher();
  }
})();
