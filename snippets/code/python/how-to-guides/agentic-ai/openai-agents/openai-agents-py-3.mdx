...
triage_agent = Agent(
    name="Triage Agent",
    instructions="""
    Determine which specialist agent (Math or History)
    to use based on the user's question. If it's not clearly
    Math or History, say you cannot help.""",
...
async def main():
    ...
    result_history = await Runner.run(triage_agent,
