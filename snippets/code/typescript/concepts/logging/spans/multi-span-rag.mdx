...
const generationWorkflow = logger.addWorkflowSpan({
  input: `
  Based on these documents, what are the health benefits of meditation?
  `, // input
  output: undefined, // output (will be set later)
  ...
  input: [
    {
      role: "user",
      content: `
      Based on these documents, what are the health benefits
      of meditation? Documents: [...]
      `,
    },
  ],
  output: {
    role: "assistant",
    content: `
    Based on the research, meditation offers several health benefits
    including reduced stress and anxiety, lower blood pressure...
    `,
  },
  model: "gpt-4o",
