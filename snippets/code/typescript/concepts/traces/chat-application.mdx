...
async function processChatMessage(userId: string, message: string, conversationHistory: string[]) {
  ...
  const trace = logger.startTrace(
    ...
    "Chat Interaction", // name
    Date.now() * 1000000, // createdAt in nanoseconds
    undefined, // durationNs
    { userId, conversationLength: conversationHistory.length }, // metadata
    ['chat'] // tags
  );
  ...
  try {
    ...
    logger.addLlmSpan({
      ...
      output: { role: 'assistant', content: "I'd be happy to help with that..." },
      model: "gpt-4o",
    ...
    logger.addToolSpan(
      ...
      undefined, // output
      undefined, // durationNs
      Date.now() * 1000000, // createdAt
      { error: String(error) }, // userMetadata
      ['error', 'chat'] // tags
    );
