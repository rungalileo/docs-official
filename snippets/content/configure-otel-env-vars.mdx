...
These environment variables are consumed by the galileo.otel helpers
(such as `GalileoSpanProcessor`) to authenticate
...
and metadata using your environment variables. It also sets up
...
# Create the Galileo span processor
galileo_span_processor = otel.GalileoSpanProcessor(
    ...
    # logstream=os.environ.get("GALILEO_LOGSTREAM"),  
)
```
