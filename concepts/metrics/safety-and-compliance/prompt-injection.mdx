...
<DefinitionCard>
  <strong>Prompt Injection</strong> is a security vulnerability in systems that rely on large language models (LLMs) where malicious input content manipulates the model to perform unintended actions or provide harmful outputs.
</DefinitionCard>

The Prompt Injection metric identifies instances of prompt injection within a model's input content, flagging several common attack patterns:
...
When using an LLM as a judge, the metric returns `true` when it detects a prompt injection attempt and `false` otherwise.
...
2. **Create response strategies**: Develop appropriate responses when prompt injection is detected
...
- Automatically identify model inputs that contain prompt injection attacks
