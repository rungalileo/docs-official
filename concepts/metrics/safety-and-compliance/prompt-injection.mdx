...
<Steps>
  <Step title="We leverage a Small Language Model (SLM) trained on a proprietary dataset augmented with public datasets" />
  <Step title="The model averages 87% detection accuracy and 89.6% accuracy on downstream classification of detected prompt injections" />
  <Step title="Testing is performed against open-source datasets such as JasperLS prompt injection, Ivanleomk's Prompt Injection, and Hack-a-prompt dataset" />
</Steps>
