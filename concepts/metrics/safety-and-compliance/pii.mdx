...
<Steps>
  <Step title="We leverage a Small Language Model (SLM) trained on proprietary datasets." />
  <Step title="The model scans both input and output text for specific PII patterns." />
  <Step title="Detected PII is categorized by type and confidence level." />
  <Step title="Results are presented with highlighting to show exactly where PII was detected." />
</Steps>
