...
<Steps>
  <Step title="At the token level, it measures the model's confidence in predicting the next token given preceding tokens" />
  <Step title="At the response level, it takes the maximum token-level uncertainty across all tokens in the response" />
  <Step title="The calculation uses OpenAI's Davinci models or Chat Completion models (available via OpenAI or Azure)" />
</Steps>
