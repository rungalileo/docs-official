---
title: Chunk Attribution Utilization
...
Chunk Utilization is calculated from the portions of each retrieved chunk that influence the model's response. A chunk with low utilization contains "extraneous" text that did not affect the final response, indicating potential inefficiencies in your chunking strategy.
