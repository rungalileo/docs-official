...
- **Generation Quality** – Evaluate whether the model uses the context effectively and grounds responses in retrieved context (chunk relevance, context adherence, completeness). For ground truth adherence, correctness, and instruction adherence — which apply to both RAG and non-RAG use cases — see [Response Quality metrics](/concepts/metrics/response-quality/response-quality-overview).
...
**Also check:** [Chunk Relevance](/concepts/metrics/rag/retrieval-quality/chunk-relevance) to see which chunks actually influenced the answer and which were ignored.
</Accordion>
...
**Start with:** [Chunk Relevance](/concepts/metrics/rag/retrieval-quality/chunk-relevance) to see exactly which chunks contributed to the response.
...
- **Chunk Relevance** reveals which specific chunks actually influenced the response.
