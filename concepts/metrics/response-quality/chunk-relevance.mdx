...
<Scale 
  low="0" lowLabel="Low Relevance" 
  mid="0.5" midLabel="Mid Relevance"
  high="1" highLabel="High Relevance" 
...
<Steps>
  <Step title="The model is a transformer-based encoder trained to identify relevant information in the provided query, context, and response" />
  <Step title="The same model is used to compute Chunk Adherence, Chunk Completeness, Chunk Attribution, and Utilization" />
  <Step title="A single inference call is used to compute all the metrics at once" />
  <Step title="For each token in the provided context, the model outputs a relevance probability (the likelihood that this token is useful for answering the query)" />
  <Step title="The model is trained on carefully curated RAG datasets and optimized to closely align with the RAG Plus metrics" />
</Steps>
