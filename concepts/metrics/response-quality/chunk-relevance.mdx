...
<Steps>
  ...
  <Step title="Prompt Engineering">
    A carefully engineered prompt is used to evaluate whether each chunk is explicitly related to the query and answers it fully or partially, based only on what is explicitly stated in the query and chunk.
...
<CardGroup cols={2}>
  <Card title="Combine with Other Metrics" icon="link">
    Chunk Relevance works alongside [Context Precision](/concepts/metrics/response-quality/context-precision), [Precision @ K](/concepts/metrics/response-quality/precision-at-k), and [Chunk Attribution Utilization](/concepts/metrics/response-quality/chunk-attribution) for a comprehensive view of retrieval effectiveness.
