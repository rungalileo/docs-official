...
<Note>
Chunk Relevance does not require the chunk to fully answer the query—even partial relevance is sufficient. Do not penalize for incompleteness; as long as something in the chunk is relevant, label it as Relevant.
</Note>
...
<Steps>
  <Step title="Model Request">
    Additional evaluation requests are sent to OpenAI's GPT4o model to analyze each chunk's relevance to the user query.

</Step>
  <Step title="Prompt Engineering">
    A carefully engineered prompt asks the model to evaluate whether each chunk is explicitly related to the query and answers it fully or partially, based only on what is explicitly stated in the query and chunk.
...
Consider this example that illustrates chunk relevance:
...
<div style={{ marginTop: "1rem", paddingTop: "0.75rem", borderTop: "1px solid rgba(209, 213, 219, 0.33)" }}>
  <strong>Relevance analysis:</strong> Chunks 1 and 2 would be <Pill label="Relevant" /> because they provide information directly related to answering the query. Chunk 1 provides crucial context (the capital is Paris) that enables the answer to be found, and Chunk 2 provides the actual answer. Chunk 3 would be <Pill label="Not Relevant" /> because it only provides general background information about France's location, which doesn't help answer the population question.
</div>
...
When many chunks are marked as <Pill label="Not Relevant" />, it indicates issues with your retrieval system. To improve your system:
...
<div style={{ marginTop: "0.75rem", paddingTop: "0.75rem", borderTop: "1px solid rgba(209, 213, 219, 0.33)" }}>
  <strong>Analyze patterns:</strong> Identify common characteristics of non-relevant chunks to understand why your retrieval system is selecting them.
</div>
...
<CardGroup cols={2}>
  <Card title="Use as Foundation Metric" icon="layer-group">
    Leverage Chunk Relevance as the base metric for calculating composite metrics like Context Precision and Precision @ K.

</Card>
  <Card title="Combine with Other Metrics" icon="link">
    Use Chunk Relevance alongside [Context Precision](/concepts/metrics/response-quality/context-precision), [Precision @ K](/concepts/metrics/response-quality/precision-at-k), and [Chunk Attribution Utilization](/concepts/metrics/response-quality/chunk-attribution) for a comprehensive view of retrieval effectiveness.

</Card>
  <Card title="Optimize Retrieval Strategy" icon="sliders">
    Use relevance scores to refine your embedding models, similarity search algorithms, and retrieval parameters.

</Card>
  <Card title="Monitor Across Queries" icon="chart-line">
    Track relevance rates across different query types to identify patterns and improve your retrieval system's performance.

</Card>
</CardGroup>

<Note>
Chunk Relevance is designed to be lenient—it marks chunks as relevant if they provide any useful information, even if incomplete. This ensures that chunks with partial answers or bridging information are not incorrectly marked as irrelevant.
