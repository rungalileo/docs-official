...
<DefinitionCard>
  <strong>Instruction Adherence</strong> measures whether a model followed or adhered to the system or prompt instructions when generating an output.
</DefinitionCard>

## How it works

This metric is particularly valuable for uncovering hallucinations where the model is ignoring instructions, which can lead to outputs that don't meet user requirements or business rules.
...
<Steps>
  <Step title="Model Evaluation">
    The system sends multiple evaluation requests to OpenAI's GPT4o model to analyze whether the output follows the provided instructions.

</Step>
  <Step title="Analysis Process">
    A specialized chain-of-thought prompt guides the model through a detailed evaluation of how well the output adheres to the specific instructions given.

</Step>
  <Step title="Multiple Assessments">
    The system requests and collects multiple distinct assessments to ensure a robust evaluation through consensus.
  ...
  <Step title="Result">
    The evaluator returns `true` when the output follows the provided instructions and `false` otherwise.

</Step>
</Steps>

We also surface one of the generated explanations, always choosing one that aligns with the majority judgment among the assessments.
...
<div style={{ marginTop: "1rem", paddingTop: "0.75rem", borderTop: "1px solid rgba(209, 213, 219, 0.33)" }}>
  <strong>Instruction Adherence:</strong> Measures whether the output follows the instructions in your prompt template.
</div>

<div style={{ marginTop: "0.75rem", paddingTop: "0.75rem", borderTop: "1px solid rgba(209, 213, 219, 0.33)" }}>
  <strong>Context Adherence:</strong> Measures whether the output adheres to the context provided (e.g., your retrieved documents).
</div>
...
<div style={{ marginTop: "1rem", paddingTop: "0.75rem", borderTop: "1px solid rgba(209, 213, 219, 0.33)" }}>
  <strong>Flag and examine non-compliant outputs:</strong> Identify patterns in outputs that don't follow instructions.
</div>
...
<div style={{ marginTop: "0.75rem", paddingTop: "0.75rem", borderTop: "1px solid rgba(209, 213, 219, 0.33)" }}>
  <strong>Implement guardrails:</strong> Take precautionary measures to prevent non-compliant outputs from reaching end users.
</div>
