...
Context Precision is computed using [Chunk Relevance](/concepts/metrics/response-quality/chunk-relevance) scores:
...
<Steps>
  ...
  <Step title="Numerator Calculation">
    The numerator is calculated as the sum of (1.0 if the chunk is relevant, else 0.0) divided by (i + 1), where i is the position index starting from 0.

</Step>
  <Step title="Denominator Calculation">
    The denominator is the sum of 1/(i+1) for all chunks, where i is the position index starting from 0.
...
<Note type="info">
  Context Precision is differentiated from [Precision @ K](/concepts/metrics/response-quality/precision-at-k): Context Precision considers all retrieved chunks to measure noise in retrieval, while Precision @ K evaluates precision at a specific rank K and helps assess ranking quality.
</Note>
