...
[_Context Adherence Luna_](TBA) is computed using Galileo in-house small language models (Luna). They're free of cost, but lack 'explanations'. Context Adherence Luna is a cost effective way to scale up you RAG evaluation workflows.

[_Context Adherence Plus_](TBA) is computed using the [Chainpoll](https://arxiv.org/abs/2310.18344) technique. It relies on OpenAI models so it incurs an additional cost. Context Adherence Plus has shown better results in internal benchmarks. Additionally, _Plus_ offers explanations for its ratings (i.e. why something was or was not adherent).
