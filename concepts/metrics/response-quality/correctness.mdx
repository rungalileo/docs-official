...
<Scale 
  low="0" lowLabel="Low Correctness" 
  high="1" highLabel="High Correctness" 
  lowDescription="The response contains factual errors" 
  highDescription="The response is factually accurate"
/>
...
<Steps>
  <Step title="Sending additional requests to OpenAI's GPT4-o" />
  <Step title="Using a carefully engineered chain-of-thought prompt that asks the model to judge whether the response is factually accurate" />
  <Step title="Requesting multiple distinct responses to this prompt" />
  <Step title="Each evaluation produces an explanation along with a final judgment: yes or no" />
  <Step title="The final score is the fraction of 'yes' responses divided by the total number of responses" />
</Steps>
...
<Card>
  ...
  <div style={{marginTop: '1rem', paddingTop: '0.75rem', borderTop: '1px solid rgba(209, 213, 219, 0.33)'}}>
    <strong>Example:</strong> In a text-to-SQL scenario, a response could be factually correct (high Correctness) but not derived from the provided context (low Context Adherence). Conversely, a response could faithfully represent the context (high Context Adherence) but contain factual errors if the context itself is incorrect.
  </div>
</Card>
