---
title: Correctness
description: Evaluate factual accuracy in AI outputs using Galileo Guardrail Metrics
---
...
<DefinitionCard>
  <strong>Correctness</strong> measures whether a model response is factually accurate. It evaluates factual claims, labels, and identifiable entities where applicable.
</DefinitionCard>

<SupportedModalities modalities={["Text", "Image/PDF", "Audio"]} />

Correctness is a boolean metric that indicates whether the response is factually accurate.

This metric is particularly valuable for uncovering open-domain hallucinations and other factual inaccuracies that don't relate to any specific documents or context provided to the model.
...
<Steps>
  ...
  <Step title="Result">
    The evaluator returns `true` when the response is factually accurate and `false` otherwise.
...
- If the result is `true`, the explanation provides an argument that the response is factual.
- If the result is `false`, the explanation provides an argument that it is not factual.
...
<BooleanClassificationReport
  ...
  False       0.8494    0.7199    0.7793       282
   True       0.8701    0.9363    0.9020       565

accuracy                          0.8642       847
macro avg     0.8597    0.8281    0.8406       847
weighted avg  0.8632    0.8642    0.8611       847`}
  negativeLabel="False"
  positiveLabel="True"
  negativeClass="False"
  positiveClass="True"
