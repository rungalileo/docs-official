...
- **In the UI**: Select metrics from the "Required Metrics" dropdown
...
**Required metrics to select** (in UI dropdown or SDK parameter):
...
Composite metrics are executed after all their required metrics have been
computed. Plan your metric dependencies to avoid circular references.

### Handle missing data gracefully

Always check for empty lists or missing spans when aggregating:
