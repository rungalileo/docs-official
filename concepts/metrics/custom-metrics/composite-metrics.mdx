...
- **Boolean metrics** (e.g. `correctness`, `context_adherence`) are evaluated by multiple judges and return a `list[int]` at the root level, where each element is `0` (false) or `1` (true) — one value per judge.
- **Float metrics** (e.g. `completeness`) return a single `float` between 0 and 1.
