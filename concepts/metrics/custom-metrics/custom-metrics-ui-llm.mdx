...
import SnippetExperimentPython from "/snippets/code/python/sdk/experiments/custom-metric.mdx";

import SnippetExperimentTypeScript from "/snippets/code/typescript/sdk/experiments/custom-metric.mdx";
...
The response needs to be a fixed type for the metric to be evaluated correctly. Currently the following response type is supported:

- **Boolean (`true` or `false`)** - the prompt needs to return `true` or `false` only. 
...
In the Galileo console, go to the Metrics hub and click the "Create New Metric" button in the top right corner.
...
When you add a metric to a Log stream, you can configure which version is used - either the default, or a specific version. If you use the default, then the version used will change as the default version changes.
