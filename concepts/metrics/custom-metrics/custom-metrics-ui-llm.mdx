...
To test against current logs or experiments, select the project, then select the source type, then select the relevant Log stream or experiment. You can then run against the last 5 logged sessions, traces, spans, or experiments by selecting the **Test Metric** button. The metric will be calculated, along with an explanation if you have step-by-step reasoning turned on.
