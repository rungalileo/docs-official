...
The response needs to match the output type selected for the metric. Currently the following output types are supported:
...
| Freeform    | Any text                                            | A prompt that returns detailed analysis or commentary instead of a value that Galileo aggregates. |

For advanced use cases, Freeform can also return custom structured text. How reliably it follows the requested structure depends on the capabilities of the LLM selected for the metric.
...
<Step title="Test your metric">
  Before saving, test the metric against real inputs and iterate on the prompt,
  model, or number of judges. From the **Test Metric** tab you can test three ways:
  with **manual input**, against your **current logs**, or against a
  **labeled dataset** to measure how closely it matches your ground truth with a macro F1 score or RMSE.
