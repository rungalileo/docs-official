...
You can create a registered custom metric either through the Python SDK or directly in the Galileo Console.
...
You need to define 2 functions that are used by the metric, one to score the metric, and one to aggregate the results at an experiment level. You can also optionally define additional functions and properties that are used by the custom metric. In addition you can define whatever additional functions and variables that you need.

#### The scorer function

This function evaluates individual responses and returns a score. The score can be a number (int or float), a boolean, string, or None.
