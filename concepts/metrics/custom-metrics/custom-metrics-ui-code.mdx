...
```python
def scorer_fn(*,
              step_object: Session | Trace | WorkflowSpan | AgentSpan | LlmSpan | RetrieverSpan | ToolSpan,
              **kwargs: Any) -> float | int | bool | str:
...
Registered custom metrics run in a sandboxed Python 3.10 environment with only the Python standard library and 'galileo' installed.

To install your own pypi package, you can define dependencies at the top of the file using the scrip dependency format from 'uv':
...
For full documentation on defining dependencies, check out the 'uv' script dependency docs: https://docs.astral.sh/uv/guides/scripts/#creating-a-python-script .
