...
![Code editor](/images/g2/custom-metrics-code-editor-ai-assisted.png)
...
<Step title="Test your metric">
    <span id="test-your-metric"></span>
  Before saving, test the metric against real inputs and iterate on the code.
  From the **Test Metric** tab you can test three ways: with **manual input**,
  ...
  <Info>
    See [Test your metrics](/concepts/metrics/custom-metrics/test-metrics) for the
    full walkthrough of each method and how the scores are calculated.
  </Info>
</Step>
<Step title="Save your metric">
After writing your custom metric code, select the **Save** button in the bottom right corner of the code editor. Your metric will be validated. If there are no errors, the metric will be saved and become available for use across your organization.
...
Writing a scorer from scratch can be tricky, especially when you're just getting started or when you're migrating from another evaluation framework. Galileo's **Help me write** feature generates a working scorer function from a plain-English description — so you can go from idea to runnable metric in seconds.

#### How to use it

<Steps>
<Step title="Create a custom code-based metric">
Follow steps 1-3 of [Create a registered custom metric](#create-a-registered-custom-metric) to start creating a custom code-based metric.
</Step>
<Step title="Enable Help me write">
In the code editor, toggle on **Help me write** above the editor panel.
</Step>
<Step title="Describe your metric">
In the prompt field, describe what you want the metric to do in natural language. Be as specific as you like — the more detail you provide, the better the generated code will match your intent. Optionally include examples of expected inputs and outputs for edge-case guidance.

You can also paste in code from another evaluation framework (LangSmith, RAGAS, etc.) and the generator will convert it to a Galileo scorer automatically.

![AI-assisted code generation](/images/g2/custom-metrics-ai-assisted-code-generation.png)
</Step>
...
Next, you can [test and save your code-based metric](#test-your-metric).
...
```text
The metric should calculate the edit distance between
...
1. When creating a code-based custom metric, use the **Composite Metrics**
   ...
   metric runs
    ...
    <img src="/images/g2/composite-metrics-dropdown.png" alt="Composite Metrics dropdown" width="520" />
