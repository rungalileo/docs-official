...
<Steps>
  <Step title="Sending multiple evaluation requests to an LLM (e.g., OpenAI's GPT4o-mini)" /> 
  <Step title="Using a specialized chain-of-thought prompt that follows the definition above" />
  <Step title="Each evaluation produces an explanation and a binary judgment (yes/no)" />
  <Step title="The final score is the percentage of 'yes' responses out of all responses" />
</Steps>
