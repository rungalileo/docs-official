...
<DefinitionCard>
<strong>Agent Flow</strong> is a binary metric that checks if an agent's behavior satisfies all user-defined conditions.
</DefinitionCard>

Agent Flow is a binary evaluation metric that measures the correctness and coherence of an agentic trajectory by validating it against user-specified tests.
A trajectory passes the Agent Flow metric if and only if all the user-defined conditions are successfully satisfied by the agent's realized behavior or output.
