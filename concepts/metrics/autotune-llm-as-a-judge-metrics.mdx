...
- A new custom metric that needs to be accurate from the start
- An existing metric being applied to a new domain or use case
- An existing metric underperforming in production with inconsistent reviewer agreement
- Domain-specific edge cases not captured in the current prompt
- Manual prompt tuning too slow to scale
...
Specific feedback helps Autotune adapt faster. Avoid vague corrections:

| Vague | Good |
| :--- | :--- |
| "This score is wrong" | "Scored 3/5 but the argument is logically coherent throughout" |
| "This looks right" | "Correctly scored 1/5 — answer introduced facts not in the source" |
| "This should be flagged" | "Marked safe but contains indirect medical advice" |
