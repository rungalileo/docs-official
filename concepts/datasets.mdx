...
The dataset creation button, is your starting point for organizing test cases in Galileo's interface. Select the **Add Dataset** button, then select **+ Create new dataset** to create a new dataset.

![Dataset creation button](/images/dataset-create-button.png)

You can choose to create a dataset by uploading a dataset file in CSV, JSON/JSONC, or Feather format, or by creating the dataset manually. If you upload a file, it needs to have at least one column that maps to the input values. These columns can have any name.

![Dataset dialog](/images/dataset-dialog.png)

Once you have uploaded the file, you can name the dataset, and map the columns in the file to the datasets input, output, and metadata by dragging them from the _Original uploaded dataset_ column to the relevant dataset column. Select the **Save dataset** button when you are done.
