...
Custom functions can be as complex as required, including multiple steps, agents, RAG, and more. This means you can build experiments around an existing application, allowing you to run experiments against the full application you have built, using datasets to mimic user inputs. For example, if you have a multi-agent LangGraph application, you can log every stage in the agentic flow as part of an experiment.
...
When functions in your application is run by the `run_experiment` call, a logger is created by the experiment runner, and a trace is started. This logger can be passed through the application, accessed using the `@log` decorator, or by calling `galileo_context.get_logger_instance()`. You will need to change your code to use this instead of creating a new logger and trace.
...
If there isn't a current logger, one will be created so this will always return a logger.
...
Once you have this information, you can use it to decide to create a new trace in your application.
