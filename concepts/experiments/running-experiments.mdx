...
You can also use the `@log` [decorator](/sdk-api/python/logging/log-decorator) or [function wrapper (TypeScript)](/sdk-api/typescript/logging/log-function-wrapper) to capture function calls in your custom function experiments. This example simulates an LLM call, so no API credits are used while you test the flow of your LLM application. You can peek at the [full code sample](#fictional-world-agent-full) for this experiment, then return to review the steps for a breakdown.
