...
Experiments are run for a specific project. They live in that project, with new log streams automatically created for each experiment.

This takes a dataset, and can either pass it to a prompt template, or to a custom function. This custom function can go from a simple call to an LLM right up to a full agentic workflow. For each row in a dataset, a new trace is created, and either the prompt template is logged as an LLM span, or every span created in the custom function is logged to that trace.

<Note>
When you call your application code from an experiment, the experiment runner will start a new trace for every row in your dataset. You will need to ensure your application code doesn't start a new trace manually. If you are using the `log` wrapper or a third-party integration, this is handed for you. If you are logging manually you will need to check to see if an experiment is in progress. See the [Experiment SDK docs](/sdk-api/experiments/running-experiments#get-an-existing-logger-and-check-for-an-existing-trace) for details on how to do this.
</Note>
...
You can learn more about running experiments in code, and get started, from the [Experiments SDK documentation](/sdk-api/experiments/experiments).
