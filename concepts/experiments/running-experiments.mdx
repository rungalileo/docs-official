...
You can use the `@log` [decorator](/sdk-api/python/logging/log-decorator) or [function wrapper (TypeScript)](/sdk-api/typescript/logging/log-function-wrapper) to capture function calls in your custom function code experiments. The `@log` decorator or function wrapper enables you to identify functions that should generate unique spans in your application, like LLM calls or tool usage. This approach can be used to simulate an LLM call, so no API credits are used while you test the expected flow of your LLM application. 
