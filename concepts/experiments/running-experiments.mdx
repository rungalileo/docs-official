...
import FindWorldCustomDBPython from "/snippets/code/python/concepts/experiments/find-world-mock-db.mdx"
import FindWorldCustomDBTypeScript from "/snippets/code/typescript/concepts/experiments/find-world-mock-db.mdx"
import FindWorldLogRunnerPython from "/snippets/code/python/concepts/experiments/find-world-log-runner.mdx"
import FindWorldLogRunnerTypeScript from "/snippets/code/typescript/concepts/experiments/find-world-log-runner.mdx"
import FindWorldLogFullPython from "/snippets/code/python/concepts/experiments/find-world-log-full.mdx"
import FindWorldLogFullTypeScript from "/snippets/code/typescript/concepts/experiments/find-world-log-full.mdx"
import FindWorldLogImportsPython from "/snippets/code/python/concepts/experiments/find-world-log-imports.mdx"
...
### Using the `log` decorator or function wrapper

You can also use the `@log` [decorator](/sdk-api/python/logging/log-decorator) or [function wrapper (TypeScript)](/sdk-api/typescript/logging/log-function-wrapper) to capture function calls in your custom function experiments. This example simulates an LLM call, so no API credits are used; it is a great way to test the flow of your LLM application. You can peek at the [full code sample](#fictional-world-agent-full) for this experiment, then return to review the steps for a breakdown.

Let's begin by importing everything that we will need, and defining a data source for the function. 

<CodeGroup>
    <FindWorldLogImportsPython />
...
<CodeGroup>
  <FindWorldCustomDBPython />
  <FindWorldCustomDBTypeScript />
</CodeGroup>


<Steps>
    <Step title="Create a tool">
        We'll create a simple regex function to find a matching world from the list, and use the `@log` decorator to identify the function as a tool:
    ...
    <Step title="Create an LLM call">
        Next, let's create a simple function to simulate our LLM call. It will use the tool we just defined to search for a world and return a response: 
    ...
    <Step title="Create the custom experiment function">
        ...
        <CodeGroup>
            <FindWorldLogRunnerPython />
            <FindWorldLogRunnerTypeScript />
        </CodeGroup>
    </Step>
...
When the experiment runs, your terminal will print out each response from the LLM, and conclude with a URL to view the captured traces in the [Galileo Console](https://app.galileo.ai). 

<span id="fictional-world-agent-full" />
<Accordion title="Log function code (select to expand)">
    <CodeGroup>
        <FindWorldLogFull />
    </CodeGroup>
</Accordion>
