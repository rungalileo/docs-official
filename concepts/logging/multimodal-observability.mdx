...
![An audio trace in the Galileo Log stream showing an inline waveform player in the user input, a text output from the assistant, and audio quality metrics in the side panel](/images/concepts/logging/multimodal-in-traces.webp)
...
![The custom metric editor showing Audio modality selected, an LLM model configured, and a judge prompt for evaluating audio quality](/images/concepts/logging/metric-creation.webp)

1. Go to **Metrics** and create a new custom LLM metric.
2. Under capabilities, select **Image/PDF** or **Audio**.
3. Configure a model integration. See [suggested models](#suggested-models) below.
