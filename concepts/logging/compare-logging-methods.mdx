...
The `galileo_context` context manager provides a way to control logging for a block of code. It is only available in the [Python SDK](/sdk-api/python/logging/galileo-context), and allows you to start logging quickly while handling details like creating traces or saving events to the Galileo Console for you. 
...
The Galileo Context manager is great when your logic is contained in a single block of code, or you are using the Python SDK and want to quickly get started logging. It is also ideal for experimenting in Python Notebooks (e.g. Jupyter). However, it cannot be used in TypeScript projects.
