...
[Python](/sdk-api/python/logging/galileo-logger) and [TypeScript](/sdk-api/typescript/logging/galileo-logger) SDKs. It lets you decide when your logging session starts or ends, create spans and traces manually within your logging session, and send a variety of other data to Galileo console. 

`GalileoLogger` supports various span types (LLM, retriever, tool, workflow, agent), as well as [logging session management](/concepts/logging/sessions/using-sessions#steps).
...
The `@log` decorator ([Python](/sdk-api/python/logging/log-decorator)) or `log` function wrapper ([TypeScript](/sdk-api/typescript/logging/log-function-wrapper)) is the most granular approach to capturing events in your application. It is used to capture the inputs and outputs of individual functions&mdash;even generator functions!&mdash;in a [span](/concepts/logging/spans). 
...
View the [Python](/sdk-api/python/logging/log-decorator) or [TypeScript](/sdk-api/typescript/logging/log-function-wrapper) docs for more information. 

### When to use `log`

This approach is ideal for generating spans in your logs (e.g. an LLM response, or a call to a helper function), and is best used on functions that represent meaningful work units. Note that, if your application has a lot of units to be manually logged, this approach will require a little more work than the others. 
