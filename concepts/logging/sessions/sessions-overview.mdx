...
<DefinitionCard>
A Session is **a collection of Logs, Events, and Spans emitted by your Application**. They group all Traces for one conversation or evaluation run, giving you a bird's-eye view of that LLM workflow.
</DefinitionCard>

## Introduction

Imagine you're building a simple LLM-powered customer service chat application. In testing it, you might want to see how a single conversation flows from user prompt to tool calling to model response. **Sessions** solve this problem by bundling low-level events into a cohesive unit, so you can trace an entire LLM interaction from start to finish.
...
<InfoNote>You can use **Sessions** to segment a log stream into coherent experiments or runs, making it easier to analyze end-to-end behavior.</InfoNote>
