...
<Note>
Treat `logger.start_session` like a lifecycle event, and call it before any code you want to monitor. The `name`, `external id`, and `metadata` arguments are all optional.
</Note>
</Step>
...
1. Use the TypeScript SDK's `getLogger` function, or the Python SDK's `galileo_context` context manager for a consistent reference:
    ...
    galileo_context.start_session(
        name="My Session",
        metadata={"brand_id": "acme", "env": "production"},
    )
