...
Your conversations are saved, so you can pick up an investigation later. Each one starts fresh, with no memory carried between conversations, so keep the context you need inside the thread you're working in.

## Limitations

The assistant is in Beta and read-only today. A few things to keep in mind:

* **It answers and recommends, but it doesn't act yet.** It can recommend a fix and show you where to make it, but you'll need to apply the changes yourself.
* **No memory across conversations.** Each conversation starts fresh with no context from previous threads.
