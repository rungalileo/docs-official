...
The **Add dataset** modal is reused everywhere you can pick a dataset source — the Datasets page, [Step 2 of the run creation flow](/luna-studio/runs/new-run/step-2-test-set), and [Step 3](/luna-studio/runs/new-run/step-3-training-set). The title and copy adapt to whether you're adding a test set or a training set, but the three sources are the same.

<Frame caption="Add new dataset modal — pick a source: local upload, URL, or Galileo import">![Add dataset modal](/images/luna-studio/datasets/add-dataset-modal.png)</Frame>
...
<Frame caption="Fetch from URL — paste an https://, s3://, or gs:// link">![Add dataset, URL](/images/luna-studio/datasets/add-dataset-url.png)</Frame>
...
- **`s3://`** &mdash; uses the credentials in your [AWS-hosted models integration](/luna-studio/integrations/llm-providers#aws-hosted-models), if any. For public buckets, no auth is needed.
