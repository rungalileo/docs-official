...
<Frame caption="Datasets page with the Test sets and Training sets tabs">
  ![Datasets page](/images/luna-studio/datasets/datasets-home.png)
</Frame>
...
Click any row to open the [Dataset details](/luna-studio/datasets/dataset-details) page.
...
```mermaid
flowchart LR
  TestSet1["Test set: rag-eval-v2"] --> Run1["Run 1: gpt-5 generation"]
  TestSet1 --> Run2["Run 2: claude generation"]
  TrainSet1["Training set: gen from rag-eval-v2 (gpt-5)"] --> Run1
  TrainSet2["Training set: gen from rag-eval-v2 (claude)"] --> Run2
```
...
- **Generated** &mdash; produced by the [Generate from test set](/luna-studio/runs/new-run/step-3-training-set#generate-from-test-set) flow inside the new-run wizard.
...
<CardGroup cols={2}>
  ...
  <Card title="Training sets" icon="dumbbell" href="/luna-studio/datasets/training-sets">
    What training sets are and the four ways to create them.
  </Card>
