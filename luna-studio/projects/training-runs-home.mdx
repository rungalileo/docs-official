...
<Frame caption="Training runs home for a single project, with status counts and the runs table">
  ![Training runs home](/images/luna-studio/projects/training-runs-home.png)
</Frame>
...
| **New run**   | Primary button on the right. Opens the [New run wizard](/luna-studio/runs/new-run/overview). |

<Tip>
  The **Starred** filter is the fastest way to surface runs you're actively iterating on. Star a row
  from the leftmost column of the table — see below.
</Tip>
...
Click any row to open the [Run details](/luna-studio/runs/run-details) page.
...
<CardGroup cols={2}>
  ...
  <Card title="Run details" icon="circle-info" href="/luna-studio/runs/run-details">
    Inspect a run's status, metrics, and metadata.
  </Card>
