---
title: Config file
description: "Complete reference for the data generation section of the config."
---
...
| `metric.input_format`        | `string`                    | Yes           | —       | One of `single`, `tuple`, `rag`, `span_with_tools`, `trace`, or `session`. `trace` and `session` require label-only mode or skipping data generation. |
...
| Field             | Type     | Required | Default (base config)      | Notes                                                               |
| ----------------- | -------- | -------- | -------------------------- | ------------------------------------------------------------------- |
| `provider`        | `string` | Yes      | `"openai"`                 | One of `anthropic`, `azure`, `gemini`, `groq`, `openai`, or `vegas`. |
...
| Field                                  | Type                | Required | Default (base config) | Notes                                                                                  |
| -------------------------------------- | ------------------- | -------- | --------------------- | -------------------------------------------------------------------------------------- |
| `context_examples`                     | `int`               | Yes      | `5`                   | Number of seed examples placed in the prompt per call. For `rag` and `span_with_tools`, must be `1`. |
...
Because the default is `5`, a `span_with_tools` generation config must override it explicitly:
