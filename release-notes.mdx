...
- **Python SDK**: [`extend_dataset`](https://v2docs.galileo.ai/sdk-api/python/reference/datasets#extend-dataset) function enables programmatic creation of synthetic data to extend existing datasets with generated examples based on prompt settings, instructions, examples, and data types.
- **TypeScript SDK**: [`extendDataset`](https://v2docs.galileo.ai/sdk-api/typescript/reference/README/functions/extendDataset) function provides equivalent functionality for TypeScript applications, allowing developers to extend datasets with synthetically generated data using configurable parameters for model settings, prompts, and data generation criteria.
...
Galileo now supports [synthetic data generation](/concepts/datasets#synthetic-data-generation), allowing you to create training and evaluation datasets via the UI. This feature enables you to generate diverse, controlled datasets for testing your AI applications without manual data collection.
...
Apply these metrics at [span](/concepts/logging/spans#spans), [trace](/concepts/logging/traces#traces), or [session levels](/concepts/logging/sessions/sessions-overview), or create agentic metrics to evaluate complete workflows.
