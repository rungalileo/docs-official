---
title: Creating Custom Metrics for Domain-Specific Applications
description: Learn how to create custom metrics for domain-specific appliciations.
tag: "Python"
---

## Overview

In this tutorial, you'll learn how create custom metrics for domain-specific applications within Galileo.   We'll start with an existing agentic application, add agent-specific evaluations using Galileo, and finally, customize our metric for the domain. This tutorial is intended for Python developers who already have an app and are looking to add evaluation. It assumes you have basic knowledge of:
...
* Create custom metrics using natural language designed for domain specificity 
...
This tutorial uses an existing comedy startup simulator Flask app powered by Galileo and OpenAI. This is a very simplistic example of a chatbot for a fictitious bank. It is a multi-agent app, with a supervisor agent, and a single additional agent that can be used to answer questions on the credit cards offered by the bank. This agent uses some dummy credit card documents stored in a Pinecone vector database.
...
<Steps>
<Step title="Create a new project from the [Galileo Console](https://app.galileo.ai/) using the **+ New Project** button.">
...
<Step title="Add the following Galileo environment variables to you `.env` file.">
