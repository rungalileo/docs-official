...
- **Automatic tracing**: OpenInference automatically traces your LangGraph workflow without requiring LLM API calls
- **Simplified workflow**: A streamlined text processing pipeline that showcases OpenTelemetry instrumentation
- **Production configuration**: Proper authentication and BatchSpanProcessor setup for efficient trace export
- **Clean span structure**: Focused on LangGraph nodes without additional complexity from LLM interactions
...
OTEL Headers: Galileo-API-Key=lukvRtWj1ApPZGRHst1Gw_eAhijJ2bBjyuTj27ywJVA,project=OTEL,logstream=langgraph
...
✓ Received response: 'Galileo Galilei discovered four moons of Jupiter, which are now known as the Galilean moons. They ar...'
✨ Parsed answer: 'Galileo Galilei discovered four moons of Jupiter, which are now known as the Galilean moons.'

=== FINAL RESULT ===
Question: what moons did galileo discover
LLM Response: Galileo Galilei discovered four moons of Jupiter, which are now known as the Galilean moons. They are Io, Europa, Ganymede, and Callisto.
Parsed Answer: Galileo Galilei discovered four moons of Jupiter, which are now known as the Galilean moons.
...
The example implements a simple two-node LangGraph workflow with automatic OpenTelemetry tracing:

```mermaid
graph TD
    A[User Input: "hello world"] --> B[Initial Node]
    B --> C[Processing Node]
    C --> D[Final Output: "Processed: HELLO WORLD"]
    
    B --> E[OpenTelemetry Span: initial_node]
    C --> F[OpenTelemetry Span: processing_node]
    
    E --> G[Automatic Tracing:<br/>- Input validation<br/>- State management]
    F --> H[Automatic Tracing:<br/>- Text transformation<br/>- Output generation]
