...
Next, we define the `setup_elasticsearch` function. This function connects to your Elasticsearch instance, deploys the ELSER model if it's not already available, and indexes some sample documents. This ensures that your RAG application has data to retrieve.

<SnippetElasticRag2 />

### Agent Definition

Here, we define the core logic of our RAG agent using LangGraph. We define the agent's state, create a retriever tool to search for documents in Elasticsearch, and build a graph that orchestrates the conversation flow.

<SnippetElasticRag3 />

### Running the Agent

The `ask_question` function is responsible for interacting with the agent. It takes a question and a session ID, sends it to the LangGraph agent, and returns the response. It also manages the conversation history, allowing the agent to have context-aware conversations.
