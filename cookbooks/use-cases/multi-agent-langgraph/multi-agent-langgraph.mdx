...
* Python
* LangGraph
* The basics of [setting up a project and metrics in Galileo](/getting-started/quickstart)
...
- `src/galileo_langgraph_fsi_agent/agents/credit_card_information_agent.py` - This is a LangGraph agent that uses a tool to extract information about the available credit cards from Pinecone.
...
1. **The starter project** - Clone the [Galileo SDK-Examples repo](https://github.com/rungalileo/sdk-examples). This repo contains both the starting LangGraph app that you will be adding Galileo evaluations to, as well as a final version for reference.
1. **A Pinecone account and API key** - If you don't have an existing Pinecone account, head to [pinecone.io](https://www.pinecone.io), sign up for a free account, and get an [API key](https://docs.pinecone.io/guides/get-started/quickstart#2-get-an-api-key).
1. **An OpenAI API key** - This example uses OpenAI as the underlying LLM to run the agents.
1. **A Galileo API key** - To access your Galileo API keys, open the [Galileo Console](https://app.galileo.ai/) and log in or create an account. From the **Settings and Users** page you can create a new API key.
...
1. Install the dependencies that are defined in the `pyproject.toml`. Create a virtual environment, and install these dependencies using a tool such as [`uv`](https://docs.astral.sh/uv/):
...
1. Configure your `.env` file. Copy the `.env.example` file to `.env`, and set the values for your OpenAI and Pinecone API keys:
...
1. Upload the dummy credit card documentation to Pinecone using the provided helper script:
...
1. Run the project to test it out:
    ...
    ![A demo of the bot responding to being asked what credit cards do you offer. The bot lists 2 cards](bot-demo.gif)
...
1. Create a new project from the [Galileo Console](https://app.galileo.ai/) using the **+ New Project** button. Name this project `bank-chatbot`.

    ![The create project dialog](new-project.webp)
...
1. Add the following Galileo environment variables to you `.env` file:
...
1. Start a Galileo session. In the `on_chat_start` function in `app.py`, add the following code to create a new logging session:
...
1. Create a callback handler. After the code you just added, add the following to create the callback handler, and save it in the Chainlit session:
...
1. Pass the callback handler to LangGraph. In the `main` function, replace this line:
...
1. View the session in Galileo. Open the [Galileo console](https://app.galileo.ai/) and select your project. In the **Sessions** tab you should see a single session created for the conversation.

    ![The sessions list in Galileo with a single session](single-session-list.webp)

1. Select the single session. It will open in the sessions view showing a flowchart

    ![The session as a flowchart showing input to agent to tool to output](single-session-flowchart.webp)

    Select the nodes in this chart to see the input and output.

### Add more traces to the session

Sessions contain multiple traces. For example, a single user conversation with your bot would be a single session, containing multiple traces for the different questions you ask the bot.

1. Ask the bot a follow up question, such as "Which card has no annual fee?"

1. Follow this up with a question that does not involve specific information about the credit cards, such as "What does APR stand for?"

1. View the session in the Galileo console.

    ![A session as a flowchart, showing Trace 1 of 3](multi-session-flowchart.webp)
...
1. Navigate to the last trace. Where you asked "What does APR stand for?", the credit card agent would not need to be used, so the flowchart doesn't show this node.

    ![A session as a flowchart without a credit card agent step](multi-session-flowchart-no-cc-agent.webp)
