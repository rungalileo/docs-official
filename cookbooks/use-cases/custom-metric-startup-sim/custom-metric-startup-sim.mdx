---
title: Startup Simulator Custom Metrics 
description: Learn how to create custom metrics to evaluate domain-specific applications within Galileo.
tag: "Python"
---
...
-API keys for: [OpenAI](https://platform.openai.com/docs/overview), [NewsAPI (free!)](https://newsapi.org/), and [Galileo (free!)](https://app.galileo.ai)
...
For the sake of jumping right into action — we’ll be starting from an [existing application](https://github.com/erinmikailstaples/startup-sim-3000) and demonstrating how to add custom metrics to an existing application. 
...
<Step title="Getting your Galileo API Keys"/>
...
![A gif showing how to create your API keys within Galileo](cookbooks/use-cases/custom-metric-startup-sim/create-api-keys.gif)

Run your app again, and you'll now be able to see the logs appear within Galileo.  If you're looking to see how to instrument each individual log within this application — follow the [full tutorial on our blog](https://galileo.ai/blog/silly-startups-serious-signals-how-to-use-custom-metrics-to-measure-domain-specific-ai-success#why-custom-metrics-matter). 

</Steps>

## Creating a custom LLM-as-a-Judge metric in Galileo
<Steps>
<Step title="Adding metrics to your log stream in Galileo"/>
...
![A gif showing where to see all metrics within Galileo](cookbooks/use-cases/custom-metric-startup-sim/configure-metrics.gif)

<Step title="Adding custom metrics"/>
...
![A screenshot with a red arrow pointing to the Create Metric button in the upper right hand corner of the screen.](cookbooks/use-cases/custom-metric-startup-sim/LLM-as-judge-Metric.webp)

<Step title="Creating your own LLM-as-a-Judge prompt"/>
...
<Step title="Testing your metric"/>

From within the Custom Metric UI, select Test Metric. 

![A screenshot showing where to test the metric within the Custom Metric UI](cookbooks/use-cases/custom-metric-startup-sim/test-custom-metric.webp)

Take the output from an earlier run, and paste it in the output section of the ‘test metric’ page, and check the response.

![A screenshot showing where to test the metric within the Custom Metric UI](cookbooks/use-cases/custom-metric-startup-sim/tested-metric.webp)
...
<Step title="Adding your Custom Metric"/>
...
### Next steps 
- Check out other cookbooks in the [Galileo cookbook library](https://v2docs.galileo.ai/cookbooks/overview)
- Explore how to create custom metrics in [Galileo using code](https://v2docs.galileo.ai/concepts/metrics/custom-metrics/custom-metrics-ui-code). 
- Learn more about the different [out-of-the-box-metrics available](https://v2docs.galileo.ai/concepts/metrics/overview) in Galileo. 