...
To evaluate metrics, you first need to set up an LLM integration.

<SnippetConfigureIntegrationSteps/>

## Configure metrics for your Log stream

The next step is to configure which metrics are evaluated against each logged trace.
...
Open the Log stream for your project by selecting the Log stream from the list in your project's dashboard.

![The Log stream on the dashboard](/getting-started/evaluate-and-improve/dashboard-log-stream-list.webp)
</Step>
<Step title="Open the configure metrics pane">
Open the configure metrics pane by selecting the **Configure Metrics** button.

![The configure metrics button](/getting-started/evaluate-and-improve/configure-metrics-button.webp)
</Step>
...
Once this is on, close the metrics hub with the X in the top right.
</Step>
</Steps>
