...
description: Learn how to evaluate metrics for your logged trace with Galileo, and improve your application
---
...
<CodeGroup>```python Python from galileo import GalileoMetrics from galileo.log_streams import enable_metrics ``` ```typescript TypeScript import {(enableMetrics, GalileoMetrics)} from "galileo"; ```</CodeGroup>
...
<CodeGroup>```bash Python python app.py ``` ```bash TypeScript npx tsx app.ts ```</CodeGroup>
