...
| `GALILEO_PROJECT`     | The name of your Galileo project - this is set to the sample project |
| `GALILEO_LOG_STREAM`  | The name of your Log stream - this is set to the default Log stream name for the sample project |
...
Navigate to the **Default Log Stream** by selecting it and selecting the **View all logs** button.

<Columns cols={2}>
  ![The view all logs button](/getting-started/sample-projects/view-all-logs-button.webp)
</Columns>
...
For some of the traces, these metrics are evaluated at 100%, showing the chatbot is working well in those situations. For other traces, these metrics are reporting lower values, showing the chatbot needs some improvements.
...
This will likely cause the chatbot to mislead users. The insights should show this:
...
To fix this, review the relevant file:
...
Try commenting out the original system prompt, and uncomment the suggestion. Then restart the chatbot and interact with it, asking questions about made-up things to see how it responds. You should see the metrics improving.

## Run experiments

Galileo allows you to run [experiments](/concepts/experiments/overview) against [datasets](/concepts/datasets) of known data, generating traces in an experiment Log stream and evaluating these for different metrics. Experiments allow you to take a know set of inputs and evaluate different prompts, LLMs, or versions of your apps.
