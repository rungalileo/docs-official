...
In the Trace, you can see the prompt and the model's response. However, there is **no data about the model's performance**. To more accurately measure our model's performance, you can add the **<Tooltip tip="Measures whether a model followed or adhered to the system or prompt instructions when generating a response. Instruction adherence is a good way to uncover hallucinations where the model is ignoring instructions.">instruction adherence</Tooltip>** Metric to your logged Traces in the following steps.
...
Using the instruction adherence Metric **requires an API key**.
...
prompt = """ 1. Explain Newton's First Law in one sentence of no more than fifteen (15) words. 2. Do not add any additional sentences, examples, parentheses, bullet points, or further clarifications. 3. Your answer must be exactly one sentence and must not exceed 15 words.
"""
