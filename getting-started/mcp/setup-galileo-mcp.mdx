...
import Config from "/snippets/code/mcp/mcp-config.mdx";
...
Configure the required environment variables for Galileo.

<EnvVars />

<Note>
  Replace `YOUR-API-KEY` with your actual API key. You can use other LLM providers by setting the appropriate environment variables.
</Note>
...
<Steps>
  <Step title="Install GitHub Copilot Extension">
  ...
  <Step title="Open MCP Settings">
   Open the Command Palette (`Cmd + Shift + P` on Mac or `Ctrl + Shift + P` on Windows/Linux) and search for **"MCP: Edit MCP Settings"**
  </Step>
  
  <Step title="Add Galileo MCP Configuration">
   Add the Galileo MCP server configuration to your MCP settings file.
   Ensure you add your actual Galileo API key.
