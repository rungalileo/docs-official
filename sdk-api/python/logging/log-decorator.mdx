...
Use the `params` parameter to add/overwrite the span's fields' values. Every span type has the fields `["name", "input", "metadata", "tags"]`, and in addition:
- llm spans have the extra fields `["model", "temperature", "tools"]`
- tool spans have the extra field `tool_call_id`.
