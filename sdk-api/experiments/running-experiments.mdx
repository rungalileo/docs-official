...
import SnippetExperimentsGeneratedOutputPython from "/snippets/code/python/concepts/experiments/generated-output.mdx";
...
<SnippetExperimentsGeneratedOutputPython />
...
| `ground_truth` | No | Expected answer — used by Ground Truth Adherence, BLEU, and ROUGE metrics |
...
- If you omit `prompt_template` and the dataset has a `generated_output` column, the generated output flow is used
- If you omit `prompt_template` and the dataset has no `generated_output` column, an error is returned
