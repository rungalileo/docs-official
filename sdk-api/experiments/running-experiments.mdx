...
## Group related experiment runs

You can organize runs into **experiment groups** from the [Galileo Console](https://app.galileo.ai) and from the SDK. In code, pass a **group name** with **`experiment_group`** on [`run_experiment`](/sdk-api/python/reference/experiments#run-experiment), [`create_experiment`](/sdk-api/python/reference/experiments#create-experiment), or filter with [`get_experiments`](/sdk-api/python/reference/experiments#get-experiments). See [Experiment groups](/sdk-api/experiments/experiment-groups) for console steps and naming tips.

**Assign a run to a named group**
...
run_experiment(
    ...
    experiment_group="Weekly regression",
)
```
...
The entry point for running experiments is a call to the run experiments function (see [`run_experiment`](/sdk-api/python/reference/experiments#run-experiment) or [`runExperiment`](/sdk-api/typescript/reference/README/functions/runExperiment) for more details).
