...
To log experiments to Galileo, you need to configure a connection to Galileo using an API key and optionally a URL for a custom deployment, as well as setting the project.
...
* To get a dataset, you need to have one already created through either the [console](https://app.galileo.ai/datasets) or the [SDK](/sdk-api/experiments/datasets#create-datasets). Ensure you have saved a dataset before running the experiment!
...
For more complex scenarios, you can use custom functions with the OpenAI wrapper. Here, you may use either a [saved dataset](/sdk-api/experiments/datasets#create-datasets) or a [custom one](#custom-dataset-evaluation)
