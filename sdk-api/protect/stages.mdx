---
title: Stages
description: Learn about defining stages for your runtime guardrails to be used during different stages in your application workflow
---

Stages allow you to map to and control guardrails at different stages in your applications workflow. For example, you can have a stage to run guardrails on the input from a user, and a different stage for the output from an agentic workflow.
...
Stages are created in code against a project, then used in a guardrail by name or Id. Stages can have one of two types, [central stages](#central-stages) or [local stages](#local-stages). You can provide the project name or project Id directly when creating a stage, or use the `GALILEO_PROJECT` environment variable.

When you create a central stage, you need to provide a prioritized list of [rulesets](/sdk-api/protect/rulesets). The order of these rulesets determines the action that is returned when run the guardrail. If any rulesets are triggered, the action from the first triggered ruleset in the list is returned.
...
Stages can also be versioned, with different versions having different rulesets. When you use a stage in your guardrails, you can specify the version to use.
...
See the [`create_stage` Python SDK docs](/sdk-api/python/reference/stages#create-stage) for more details.
...
When you create a central stage, you don't provide rulesets up front.
...
See the [`get_stage` Python SDK docs](/sdk-api/python/reference/stages#get-stage) for more details.
...
See the [`update_stage` Python SDK docs](/sdk-api/python/reference/stages#update-stage) for more details.
...
See the [`pause_stage` Python SDK docs](/sdk-api/python/reference/stages#pause-stage) for more details.
...
See the [`resume_stage` Python SDK docs](/sdk-api/python/reference/stages#resume-stage) for more details.
