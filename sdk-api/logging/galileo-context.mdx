...
1. Manage sessions
1. Automatically start and flush traces
1. Group related operations into a single trace
1. Ensure traces are flushed even in long-running applications
...
1. **Specify project and log stream explicitly**: While environment variables work, it's often clearer to specify the project and log stream in your code.

1. **Flush in long-running applications**: In applications that don't terminate (like web servers or Streamlit apps), explicitly flush traces to ensure they're sent to Galileo.

1. **Use nesting judiciously**: Nested contexts can be powerful but can also make your code harder to follow. Use them when you need to temporarily override settings.

## Related Resources

- [log Decorator](/sdk-api/logging/log-decorator) - For decorating functions with logging
- [GalileoLogger](/sdk-api/logging/galileo-logger) - For more manual control over logging
- [OpenAI Wrapper](/sdk-api/third-party-integrations/openai/openai) - For automatic logging of OpenAI calls
