---
title: Overview
description: An overview of the Galileo Python SDK.
---

import SnippetEnvSetup from "/snippets/code/multilingual/env-setup.mdx";

import SnippetSdkOpenAIBasicPython from "/snippets/code/python/sdk/wrappers/openai-basic.mdx";
import SnippetSdkWorkflowPython from "/snippets/code/python/sdk/decorators/workflow.mdx";
import SnippetSdkRetrieverPython from "/snippets/code/python/sdk/decorators/retriever.mdx";
import SnippetSdkToolPython from "/snippets/code/python/sdk/decorators/tool.mdx";
import SnippetSdkGalileoLoggerPython from "/snippets/code/python/sdk/manual/galileo-logger.mdx";
import SnippetSdkContextBasicPython from "/snippets/code/python/sdk/context/basic.mdx";
import SnippetSdkContextExplicitFlushPython from "/snippets/code/python/sdk/context/explicit-flush.mdx";
import SnippetInstallationPython from "/snippets/code/python/sdk/installation.mdx";
import SnippetInstallationOpenAIPython from "/snippets/code/python/sdk/installation-openai.mdx";
import SnippetLoadEnvPython from "/snippets/code/python/sdk/load-env.mdx";
import SnippetInstallDotEnvPython from "/snippets/code/python/sdk/install-dotenv.mdx";

import SnippetInstallationNpm from "/snippets/code/typescript/sdk/installation/npm.mdx";
import SnippetInstallationYarn from "/snippets/code/typescript/sdk/installation/yarn.mdx";
import SnippetInstallationPnpm from "/snippets/code/typescript/sdk/installation/pnpm.mdx";
import SnippetLoadEnvTypeScript from "/snippets/code/typescript/sdk/load-env.mdx";

The Galileo SDKs provide a comprehensive set of tools for logging, evaluating, and experimenting with LLM applications. Regardless of how you go about logging your AI application, you will still need to install the Galileo SDK and initialize your API keys by following the steps below.

<CardGroup cols={2}>
<Card title="Python SDK" icon="python" horizontal href="https://pypi.org/project/galileo/">
    The Galileo Python SDK on PyPI.
</Card>
<Card title="TypeScript SDK" icon="js" horizontal href="https://www.npmjs.com/package/galileo">
    The Galileo TypeScript SDK on npm.
</Card>
</CardGroup>

## Installation

<CodeGroup>
  <SnippetInstallationPython />
  <SnippetInstallationNpm />
  <SnippetInstallationYarn />
  <SnippetInstallationPnpm />
</CodeGroup>

If you want to use the OpenAI wrapper in Python, you need to install with the optional OpenAI dependencies.

<CodeGroup>
  <SnippetInstallationOpenAIPython />
</CodeGroup>

## Initialization and Authentication

You need a [Galileo API key](/references/faqs/find-keys#galileo-api-key) set as an environment variable called `GALILEO_API_KEY`. The Galileo SDK will automatically pick this up from the environment variable at run time.

You can also optionally set the following environment variables to define the project, log stream, and console URL that Galileo should use.

| Environment variable  | Description |
| :-------------------- | :---------- |
| `GALILEO_PROJECT`     | The [Galileo project](/concepts/projects) to log to. If this is not set, you will need to pass the project name in code. |
| `GALILEO_LOG_STREAM`  | The [default log stream](/concepts/logging/logstreams) to log to. If this is not set, you will need to pass the log stream name in code. |
| `GALILEO_CONSOLE_URL` | The URL of your Galileo console to log to. If this is not set, it will default to the hosted Galileo version at [app.galileo.ai](https://app.galileo.ai). |

When developing your application, you should use a `.env` file. Create or update a `.env` file with the following values as required:

<CodeGroup>
    <SnippetEnvSetup />
</CodeGroup>

You can then load the environment variables from this file:

<CodeGroup>
  <SnippetLoadEnvPython />
  <SnippetLoadEnvTypeScript />
</CodeGroup>

For Python, you will need to install `python-dotenv` if you haven't already.

<CodeGroup>
  <SnippetInstallDotEnvPython />
</CodeGroup>

<Note>The SDK automatically targets the [hosted Galileo console](https://app.galileo.ai). If you are using a deployment of Galileo on premises or in the cloud, point `GALILEO_CONSOLE_URL` to your own installation.</Note>

## Logging

The Galileo SDKs allow you to log all prompts, responses, and statistics around your LLM usage. There are three main ways to log your application:

1. **Use a wrapper** - use wrappers that integrate with common SDKs to automatically log LLM calls or agentic workflows.
1. **Use a decorator** - by decorating a function that calls an LLM with the `@log` decorator or `log` wrapper, the Galileo SDK logs all AI prompts within.
1. **Directly using the `GalileoLogger` class** - For more control over your logging, you can use the `GalileoLogger` directly. This allows you to manually create sessions, start traces, and log spans. This can be mixed with the other methods, for example accessing the logger directly inside a decorated function call to manually add spans.

### Log Experiments

Experiments are logged automatically when they are run, but you can use these same SDK concepts inside the code being run by your experiment for greater control and additional logging. This allows you to not only create distinct experiments, such as in notebooks, but to also add experiments to your production application code.

See our [run experiments with code documentation](/concepts/experiments/running-experiments) for more details.

## Next Steps

### Logging with the SDKs

<CardGroup cols={2}>
<Card title="Learn how to log experiments" icon="flask" horizontal href="/sdk-api/experiments">
    Learn how to run experiments with multiple data points using datasets and prompt templates
</Card>
</CardGroup>

### SDK Reference

<CardGroup cols={2}>
<Card title="Python SDK Reference" icon="python" horizontal href="/sdk-api/python/sdk-reference">
    The Galileo Python SDK reference.
</Card>
<Card title="TypeScript SDK Reference" icon="js" horizontal href="/sdk-api/typescript/sdk-reference">
    The Galileo TypeScript SDK reference.
</Card>
</CardGroup>
