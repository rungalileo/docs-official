...
Galileo connects to Google **Gemini Enterprise** (the [Gemini Enterprise Agent Platform](https://cloud.google.com/products/gemini-enterprise-agent-platform), previously known as **Vertex AI**) using a **Google Cloud service account JSON key**. Paste the full key file into Galileo to use Gemini models in the Playground, LLM-as-a-judge metrics, Log Stream Insights, and [Signals](https://galileo.ai/signals).

In the Galileo API and SDK, this integration is stored under the provider id `vertex_ai`.

## What you need

Galileo expects the **entire contents** of a service account key file (JSON), including fields such as `type`, `project_id`, `private_key`, and `client_email`. See the [Create or update Vertex AI integration API reference](https://docs.galileo.ai/api-reference/integrations/create-or-update-vertex-ai-integration) for the request shape.

This guide covers **generative AI model access** via the Vertex AI API (`aiplatform.googleapis.com`). It does not walk through deploying agents on Agent Runtime; see [Google's Agent Platform setup](https://docs.cloud.google.com/gemini-enterprise-agent-platform/build/runtime/setup) if you need that separately.
...
In the [Google Cloud console](https://console.cloud.google.com/projectselector2/home/dashboard), select an existing project or create a new one.

[Verify that billing is enabled](https://cloud.google.com/billing/docs/how-to/verify-billing-enabled#confirm_billing_is_enabled_on_a_project) for the project.

</Step>
<Step title="Enable the Vertex AI API">

Galileo calls Gemini models through the **Vertex AI API** (`aiplatform.googleapis.com`). Google has rebranded the product as **Gemini Enterprise Agent Platform** in the console, but this API name and endpoint are unchanged.

**Console:** Open [APIs & Services](https://console.cloud.google.com/apis/library) and enable **Vertex AI API**.
...
See [Set up a Google Cloud project for Vertex AI](https://cloud.google.com/vertex-ai/docs/start/cloud-environment) for more detail.
...
4. Enter a name (for example, `galileo-integration`) and optional description, then click **Create and continue**.

</Step>
<Step title="Grant IAM roles">

Assign at least **Vertex AI User** (`roles/aiplatform.user`) so Galileo can invoke Gemini models. See [Vertex AI access control](https://cloud.google.com/vertex-ai/docs/general/access-control).

| Role | Purpose |
| :--- | :--- |
| **Vertex AI User** (`roles/aiplatform.user`) | Required for model inference |
...
Click **Done** after assigning roles.

</Step>
<Step title="Create and download a JSON key">

1. On the [Service accounts](https://console.cloud.google.com/iam-admin/serviceaccounts) page, open the service account you created.
2. Open the **Keys** tab.
...
gcloud projects add-iam-policy-binding "${PROJECT_ID}" \
  --member="serviceAccount:galileo-integration@${PROJECT_ID}.iam.gserviceaccount.com" \
  --role="roles/aiplatform.user"

gcloud iam service-accounts keys create galileo-key.json \
  --iam-account="galileo-integration@${PROJECT_ID}.iam.gserviceaccount.com"
```
