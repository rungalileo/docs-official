...
<Steps>
  ...
  <Step title="Test Integration">
    After saving, test the integration by selecting the correct model from a Playground.
...
<ResponseField name="authentication_type" type="string">
  Authentication method: `"oauth2"` (default) or `"none"`. See [OAuth2 token format](#oauth2-token-format).
</ResponseField>

<ResponseField name="models" type="string[]" required>
  List of model names (minimum 1).
</ResponseField>

<ResponseField name="endpoint" type="string" required>
  Base URL for the custom integration API. Inference requests will be sent to `{endpoint}/chat/completions`.
</ResponseField>

<ResponseField name="default_model" type="string">
  Default model to use. Defaults to the first model in the list.
</ResponseField>

<ResponseField name="token" type="string">
  JSON string with OAuth2 client credentials (see below). Required for `oauth2`, must not be provided for `none`.
</ResponseField>

<ResponseField name="authentication_scope" type="string">
  OAuth2 scope to request when exchanging credentials for an access token. See [OAuth2 token format](#oauth2-token-format).
</ResponseField>

<ResponseField name="oauth2_token_url" type="string">
  OAuth2 token endpoint URL (must support client credentials grant).
</ResponseField>

<ResponseField name="custom_llm_config" type="object">
...
## Troubleshooting: custom LLM handlers
