...
In Python, there are separate callbacks for synchronous and asynchronous code. If you are using the asynchronous API, use the [`GalileoAsyncCallback` callback handler](/sdk-api/python/reference/handlers/langchain/async_handler).
...
You can add custom metadata to your logs by including it in the `metadata` parameter of a LangChain runnable config when invoking a chain or LLM.
